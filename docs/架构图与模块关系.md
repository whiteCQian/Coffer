...
以上是当前代码依赖，不是最终部署要求。MinIO 上游仓库已归档；目标版本改为三种形态共用本地受控文件系统适配器，最终程序和发布包中移除 MinIO；Redis 继续作为可选、可重建服务。服务器/NAS 的文件卷必须是应用私有卷并独立备份。
...
flowchart TB
  ...
  PORT --> FS[私有文件系统卷：每个安装实例独立]
  APP -. 可选可重建 .-> REDIS2[(Redis：会话记忆/向量)]
  LEGACY[MinIO 旧实例] -. 仅在切换窗口取证与清理，不进入最终应用 .-> RETAIN[受限回滚快照]
  UI[Web/Electron UI] --> API[受鉴权 API / 文件流]
  API --> APP
```

身份和 owner 由服务端会话派生。管理员不具备通过应用读取用户内容的权限；正文只经授权文件流接口返回，不能放在 Web 静态根目录。桌面、服务器、NAS 各自使用独立数据库、卷、密钥和备份；无跨实例同步。
...
|---|---|
| `FileController` `/api/files` | 列表、详情、上传、删除、重命名、改分类、重试 |
