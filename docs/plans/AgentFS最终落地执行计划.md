...
> 版本：2026-09-26 · 存储决策修订。适用目标：Windows 桌面版、服务器 Web 版和 NAS 版均交付；三种安装实例各自保存独立数据；每个实例支持多账号及管理员/普通用户隔离。用户已确认“重新分析后正式标签集合”问题解决，它只保留为发布回归用例。
>
...
>
> **工作区重叠标注说明：**“已有重叠实现”表示当前代码可复用、不要重复搭建；若同时标出“尚未完成”，则对应验收和缺失能力仍须按该任务完成判据处理，不能据此关闭任务。

## 0. 最终交付的定义

交付的是**一套共享业务核心、两种正式存储适配器、三种经过环境验收的部署形态**：

```mermaid
flowchart TB
  CORE[共同业务核心：身份/权限、文件治理、AI、搜索、台账、撤销] --> PORT[FileStoragePort 与可恢复操作状态机]
  PORT --> FS[本地文件系统适配：staging、校验、原子提交]
  PORT --> S3[MinIO 适配：对象写入、校验、补偿]
  FS --> WIN[Windows：Electron + 本地 Java + H2 + 本机文件库]
  S3 --> WEB[服务器 Web：Java + MySQL + 独立 MinIO]
  S3 --> NAS[飞牛 OS NAS：同一 Web 发布包 + 独立 MinIO]
...
- **服务器 Web / NAS**：MySQL 保存正式元数据，MinIO 保存文件正文；每个安装实例使用独立数据库、bucket、凭据和持久卷。前端使用生产静态资源，后端与 MinIO 由容器/服务管理；两者使用同一套发布包，在服务器与飞牛 OS 实机分别验收。
...
2. **仅桌面版移除 MinIO，Web/NAS 保留。** 桌面使用本地受控文件库，安装包和运行 Profile 不含 MinIO 服务、SDK 或凭据；Web/NAS 通过同一 FileStoragePort 的 MinIO 适配器保存正文，分别使用独立 bucket、持久卷和备份。官方开源仓库已归档为只读，参见[官方仓库](https://github.com/minio/minio)；R40 发布前必须落实可维护版本、更新渠道、漏洞处置与替换预案，不能直接沿用本机旧二进制。旧单用户数据不迁移；切换前快照与核验，维护窗口清理准确旧库和 bucket。
...
- 已形成 ADR-001，冻结管理员/普通用户权限边界、私有派生数据、旧单用户数据不迁移、模型授权、三实例数据独立、桌面本地文件库与 Web/NAS MinIO、路径冲突和失败语义。
...
- 基础门禁已完成。后续仍须补齐 OpenAPI 生成差异检查、依赖漏洞扫描、H2/MySQL 迁移升级矩阵，以及本地文件系统和 MinIO 两种适配器的故障注入；Redis 按可选/可重建依赖测试。OpenAPI job 应在接口契约生成稳定且无需外部服务后加入。将成功运行设为 M1 及后续阶段的合并门禁，并在分支保护中强制要求两个基础 job。
...
- **本轮重做与验收（2026-09-26）**：完成首次管理员初始化、登录/登出/改密、管理员创建/停用/启用/重置普通用户、账号审计、JDBC 持久化会话、密码哈希、CSRF、会话撤销和限流；初始管理员由一次性 `COFFER_ADMIN_SETUP_TOKEN` 建立。后端真实安全过滤链/会话/账号授权集成测试通过；同一无头 Edge 窗口完成 Alice、Bob、管理员切换的页面隔离验收（前端使用受控 API）。桌面浏览器与真实后端联跑、部署配置矩阵仍归 R51/R53。
...
- **本轮重做与验收（2026-09-26）**：H2 上验证 V1–V21 迁移；本机 MySQL 已对现有库实际启动并执行 V16–V21，修复 V16 对 Hibernate 物理索引名的依赖。迁移保留了旧业务行，旧行仍为 `owner_id IS NULL`，不会自动交给管理员。**仍未完成**清理 dry-run、可恢复快照/摘要和维护窗口切换；本次没有删除任何数据库行或对象，因此不能关闭 R11 或 M1。
...
- **本轮重写完成（2026-09-26）**：服务层校验启用普通用户，Spring Data Repository 工厂统一拦截私有数据调用，主键读写显式核对 owner，原生 owner 参数不可伪造；对象存储校验 owner 路径与固定 bucket。持久任务/事件携带 owner，启动恢复重读持久意图，定时任务逐启用普通账号执行。管理员私有 API 返回 403，只开放账号管理与数字型运行汇总。R10 服务角色授权与 R11 凭据主键一并修正。R12 专项 49 项通过；本轮全量后端回归 275 项通过（0 失败、0 错误）。MySQL 全新库/升级库矩阵、旧数据清理和 R51 三环境故障矩阵仍待完成。详见 [R12 授权验收记录](./R12-服务层与后台任务授权验收.md)。
...
- **本轮推翻并重写（2026-09-26）**：Agent 每轮工具由服务端绑定 owner/session；关键词、标签、最近上传、混合检索两路及降级结果均按 owner 约束后才进入模型。新增持久 owner 会话、owner 分区的 Redis 记忆和向量 generation；旧记忆在文件删除或版本变化后失效，引用携带 revision 并在读取时重新鉴权。双用户/管理员、会话猜测、恶意向量 ID、向量降级、记忆复用和历史引用交叉等自动验收 79 项通过；全量后端回归 275 项通过。真实 Redis/MinIO 故障、桌面/NAS 和多实例会话锁仍归 R51/R50。详见 [R13 搜索、Agent、记忆与引用隔离验收](./R13-搜索Agent记忆与引用隔离验收.md)。
...
- **本轮重写并验收（2026-09-26）**：用户逐任务确认模型目标；服务端生成 owner 绑定的不可变目标/密钥快照；异步提交固定配置版本；受限 URL 与重定向阻断；默认关闭正文日志并隔离/限期删除个人诊断。用户可查看目标并删除自己的诊断。专项 15 项和全量后端回归 275 项通过；前端类型检查与生产构建通过。真实外部模型、DNS/IP 钉扎、主密钥轮换演练和三环境标记扫描仍待环境门禁。详见 [R14 模型目标与内容日志隔离验收](./R14-模型目标与内容日志隔离验收.md)。
...
- **本轮重写并验收（2026-09-26）**：实现登录/首次初始化、独立管理员控制台、会话失效与身份切换隔离、无权访问页、用户模型隐私、个人数据导出/删除和清理队列状态。后端隐私集成 3 项通过；同一无头 Edge 窗口按 Alice→Bob→Alice→管理员→Bob 验证迟到响应、路由和页面状态隔离。真实后端和前端已通过一键启动健康检查（MySQL Flyway V16–V21、后端健康、前端 HTTP 200）。桌面壳和三环境发布验收仍待 R50–R53。详见 [R15 登录、管理员和隐私页面验收](./R15-登录管理员与隐私页面验收.md)。
...
- 定义 FileStoragePort 的写入、范围读取、stat、SHA-256、复制/移动、删除、no-overwrite 和冲突检查；桌面由本地文件系统适配器负责，Web/NAS 由 MinIO 适配器负责。上传、解析、预览、归档、撤销和文件访问只依赖端口。MinIO SDK、MinioClient 与 BucketInitializer 仅留在服务端适配及 Web/NAS 配置中；构建时将该适配器及 SDK 排除出桌面发布包，桌面运行 Profile 不创建 MinIO Bean，也不启动或连接 MinIO。业务记录保存 owner、逻辑路径、内容指纹和 revision，不存桌面绝对路径或服务端 bucket 凭据。
- 桌面文件根目录由用户数据目录绑定；按 owner 隔离逻辑命名空间。统一逻辑路径规范，并在本地适配器处理 Windows 大小写、Unicode、尾点/空格、设备名、路径穿越、符号链接和连接点；检查规范化后的实际路径始终在配置根目录内。MinIO 适配器校验 bucket/key 的 owner 映射，不直接接受客户端对象 key。
- 桌面文件写入采用同卷 staging、长度/摘要校验、flush/持久化与原子 rename；跨卷移动拆成持久化的 copy-verify-commit-cleanup 流程。Web/NAS 对象写入、复制和删除使用内容校验、幂等步骤与补偿任务；不能把对象操作宣称为 SQL 原子事务。NAS 在飞牛 OS 实机验证 MinIO 容器与持久卷，不承诺 NFS/SMB 多主写或集群共享写入。
- 不保留旧单用户 MinIO Key 的应用内兼容读取；旧数据由 R02 清理，不迁入新 owner。新 Web/NAS 初始化独立 bucket。过渡版本不能在旧对象尚未完成备份/清理前发布。
- **当前工作区部分实现（2026-09-26；不能视作 R20 完成）**：上传与归档对象 key 已加 owner 命名空间；Web 文件正文经 owner 校验的鉴权 API 流读取。MySQL Flyway V16–V21 在现有本机数据库的启动升级已实测，但这不替代存储适配器验收。**尚未实现**统一 `FileStoragePort`、桌面本地文件系统适配器、Range 读取、路径/符号链接与目录权限合同、MinIO 服务端适配边界，以及桌面排除 MinIO SDK/Profile。
- **完成判据**：三种部署均只能经鉴权接口读取正文；两个不同 owner 可有同名文件，同一 owner 的目标冲突不覆盖；恶意路径/链接/对象 key 不能越权；断电/进程强杀后操作可恢复或进入人工处理态。
...
- 分别做数据库与桌面文件库、数据库与 Web/NAS MinIO 对账：无引用文件或对象先隔离并按保留期处理，缺失正文进入可见异常，不自动把台账标成功。删除采用逻辑不可见与持久物理清理任务，定义备份保留与最终删除时间。
- **当前工作区部分实现（2026-09-26；不能视作 R21 完成）**：治理归档/撤销保留持久台账与补偿；文件删除同事务登记 owner 级 MinIO 清理任务，含租约、退避和状态。V17 清理队列迁移已通过 H2 测试并随本机 MySQL V16–V21 启动升级验证。**尚未完成**上传/导入/重命名/工作副本统一状态机、孤儿对象对账隔离与人工处理、启动恢复故障注入；通用恢复合同仍归 R21/R51。
- **完成判据**：在每个持久化步骤后强杀进程、重启，文件未丢失/覆盖/重复归档；每个失败都有可查询的状态和恢复动作，无永久 PENDING。

**R22｜治理与撤销完整性。依赖 R20、R21。**

- **已完成（2026-09-26）：**移除 `TagConfirmedEventListener → StorageArchiveService.archive` 与 `FileOperationService.changeCategory` 的直接归档旁路；旧分类写入路由返回 409，正式位置/分类变更统一走“预览→确认→操作台账”。
- **当前工作区已有重叠实现（2026-09-26，复用）**：确认后的正式标签集合会先清除旧关联、写入确认集合并核对，归档操作持久保存执行状态和补偿意图。
- **仍待完成：**台账保存操作前后的正式字段快照，至少包含名称、路径、分类、摘要、确认标签、归档态、revision 和对象 SHA-256；撤销按版本/内容校验恢复完整快照并重建检索索引。旧台账缺字段时标明“只能部分恢复”，不伪造历史数据。此部分本轮未改动。
...
- 新增按身份鉴权的文件流接口，支持浏览器预览/下载、Range、类型和安全响应头；Web/NAS 不再向用户提供默认 7 天 MinIO 预签名 URL，正文由授权接口从 MinIO 流式返回。删除或撤权后旧入口不能继续读取；桌面按相同 API 从本地文件流返回。
- 修复 CSV 导出公式前缀注入，导出仍逐项校验 owner；限制单次导出数量和资源消耗。
- **当前工作区已有部分重叠实现（2026-09-26，复用并补齐）**：文件详情不再返回存储路径或预签名 URL；新增登录保护、owner 校验的流式正文入口，并设置安全内容类型、下载策略、`no-store`、`nosniff` 和 CSP。**尚未实现/验收**Range 请求、桌面本地存储流、导出公式注入与规模限制，以及过期会话/删除后的端到端访问矩阵。
...
- 区分 liveness/readiness；监控任务积压、补偿、数据库连接、桌面文件库或 Web/NAS MinIO 的健康与容量、备份新鲜度。页面显示失败原因与下一步操作，避免“显示成功但事实不一致”。
- **当前工作区已有部分重叠实现（2026-09-26，复用并补验收）**：模型诊断默认关闭正文数据，异常与任务日志首轮改为输出错误类型/固定提示，旧模型日志正文按保留期清理。**尚未完成**密钥旋转、健康/容量/备份新鲜度监控、管理员聚合告警和依赖故障状态页面；日志脱敏扫描也未验收。
- **完成判据**：模拟依赖故障时状态/告警/页面一致；敏感标记在日志、指标、错误和管理员视图中均不可检索。

**M2 退出门槛：**本地文件系统适配器与 MinIO 适配器都通过端口合同、两账号治理及故障恢复场景；桌面构建产物无 MinIO SDK、连接或服务依赖，Web/NAS 仅通过受控适配器访问 MinIO；源码中无未受控的直接归档旁路。
...
- 同一套治理用例通过本地存储适配执行；在目标目录写临时文件并校验后安全提交，不能覆盖已有目标。文件/DB 步骤间沿用持久化意图和启动对账；源文件清理最后执行。
...
- 构建静态前端、后端镜像、MinIO 服务和可版本化 Compose；MySQL 与 MinIO 持久卷必需，Redis 为可选可重建服务。固定受支持的 MinIO 版本、镜像摘要、更新渠道、漏洞处置与替换预案，并完成授权/许可证检查；不能直接复用本机旧二进制。配置健康检查、就绪顺序、bucket 初始化和备份/恢复卷规则。后端不依赖开发机路径或 localhost 固定地址，使用最小权限数据库与 MinIO 服务账号。
- 只通过反向代理公开 HTTPS 前端/API；MySQL、Redis、MinIO API/控制台不直接暴露到非可信网络，bucket 不由静态 Web 服务器公开。生产禁用调试接口和正文请求日志。
- **完成判据**：全新服务器用发布说明一次安装、首次初始化、双账号核心流程和重启恢复；不以 Vite dev 服务作为生产前端；MinIO bucket/凭据隔离、持久卷权限与不公开边界有自动化或部署检查证据。

**R41｜Web 安全与容量。依赖 R40。**

- 配置 TLS、Cookie/CSRF、安全头、会话生命周期、上传配额、账号任务并发和 MinIO 持久卷容量预警。文件下载经 R23 的授权入口；bucket 私有，仅应用服务账号可按需读写。数据库、MinIO 与备份密钥分别管理，避免默认 root/空密码及生产明文连接配置。
- **完成判据**：外网只开放预期 HTTPS 端口；弱口令/暴力尝试受限；用户 A/B/管理员越权矩阵与 M1 相同；达到限额时返回可解释错误而非填满磁盘。

**R42｜Web 备份、恢复与升级。依赖 R40。**

- 设计 MySQL 与 MinIO bucket/持久卷的一致恢复点，连同模型加密主密钥、MinIO 凭据、配置和版本清单生成静态加密备份；至少保留一份脱离 NAS/服务器主机和生产凭据的副本。通过维护/快照窗口或持久操作屏障保证 DB 元数据与对象正文属于同一恢复点；验证对象清单及 SHA-256。Redis 会话与向量按规则清空/重建。Flyway 升级前留快照，无法向下迁移时通过整套快照恢复，不只回退旧 JAR。
- **完成判据**：在隔离服务器恢复双账号样本，文件 sha256、所有权、台账和密钥均一致；完成一次失败升级后回滚演练，记录 RPO/RTO。

**R43｜NAS 实机适配与验收。依赖 R40–R42。**

- 在用户确认的飞牛 OS 目标 NAS 记录具体设备型号、CPU 架构、文件系统、容器/卷/权限、网络和资源配置；使用与服务器 Web 相同的镜像部署，但使用该实例独有 MySQL、MinIO bucket/凭据与持久卷。验证 NAS 重启、卷重挂、空间不足、对象读写/复制/删除、网络中断、权限和备份恢复。
...
- **已有局部回归证据（2026-09-26；不代表 R50 完成）**：后端现有回归套件 275 项全部通过（0 失败、0 错误）；测试在 JDK 25.0.3/Maven 3.9.16 下执行，JUnit 临时目录显式设为工作区可写目录。此结果覆盖当前后端测试，并不包含 Windows 桌面、NAS 实机和本地文件系统适配器矩阵。

- 用同一组 TXT/PDF/DOC/DOCX/图片/损坏/加密/超限文件，在 Windows、服务器 Web、NAS 分别运行：导入→解析→预览修改→部分/全量确认→归档→搜索/引用→台账导出→单项/批次撤销→删除；分别覆盖本地文件和 MinIO 两种适配器。
...
- 在本地文件或 MinIO 对象写入、DB 提交、事件投递、归档复制、旧正文清理、撤销和备份阶段模拟进程强杀、网络断开、磁盘满、权限不足和服务重启；Web/NAS 额外覆盖 MinIO 不可用及恢复。
...
- **时间不足时可删减的范围**：可选 Redis 向量检索、额外音视频格式、跨实例同步、细粒度共享空间和高级规则引擎。桌面 MinIO 镜像与运行依赖已取消；Web/NAS MinIO 是正式存储依赖，不能延期。不能删减三环境交付、多账号隔离、数据保护、预览确认、台账撤销、受控引用、备份恢复与故障验收。

### 与旧任务的对应关系

- **C01–C16**：复用现有治理、解析、检索和模型配置成果；通过 R11–R26 补全多账号、状态一致性、来源定位和隐私边界，再由 R50 全量回归，不把旧交接文档的“完成”直接当作三环境验收结果。
- **C17**：R25 的敏感/未知内容远端发送门禁。
- **C18**：R40–R43 的服务器 Web Compose、NAS 实机部署和恢复；Compose 包含各实例独立的 MinIO 服务与持久卷。
...
- **C23**：本地归档由 R21、R32 实现；桌面 MinIO 镜像取消，以 R35 的可恢复备份代替。Web/NAS 继续使用 MinIO 主存储。
